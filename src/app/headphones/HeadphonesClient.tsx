...
export default function HeadphonesClient({
  headphones,
}: {
  ...
  const filteredAndSortedHeadphones = useMemo(() => {
    let result = [...headphones];

    // Apply filter
    if (filterBy === "wired") {
      result = result.filter((h) => h.type === "유선");
    } else if (filterBy === "wireless") {
      result = result.filter(
        (h) => h.type === "무선" || h.type === "유선/무선"
      );
    }
            ...
            <div className="p-4 border border-border rounded-lg bg-background/50">
              <div className="text-sm text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">
                  {filteredAndSortedHeadphones.length}
                </span>
                개의 헤드폰
              </div>
            </div>
