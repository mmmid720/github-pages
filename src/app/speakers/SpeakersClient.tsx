...
export default function SpeakersClient({
  ...
  const [selectedDevice, setSelectedDevice] = useState<SpeakerType | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const router = useRouter();

  const handleCardClick = (speaker: SpeakerType) => {
    setSelectedDevice(speaker);
    setIsModalOpen(true);
  };
  ...
  const filteredAndSortedSpeakers = useMemo(() => {
    ...
    // Apply filter
    if (filterBy === "wired") {
      result = result.filter((s) => s.type === "유선");
    } else if (filterBy === "wireless") {
      result = result.filter(
        (s) => s.type === "무선" || s.type === "유선/무선"
      );
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative z-10">
        ...
        <main className="max-w-7xl mx-auto px-6 py-12 pt-32">
          <div className="mb-8">
            <h2 className="text-3xl font-bold mb-2">스피커 카탈로그</h2>
            <p className="text-muted-foreground">
              다양한 스피커들을 둘러보고 비교해보세요.
            </p>
          </div>
